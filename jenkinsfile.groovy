pipeline {
    ...
    steps {
        script {
            if (env.COMPOSE_FILE == 'docker-compose.yml') {
                // Deploy using docker-compose.yml
                sh 'COMPOSE_HTTP_TIMEOUT=480 docker-compose up -d'
            } else if (env.COMPOSE_FILE == 'docker-compose-blue.yml') {
                // Deploy using docker-compose-blue.yml
                sh 'COMPOSE_HTTP_TIMEOUT=480 docker-compose -f docker-compose-blue.yml up -d'
                ...
                error "Invalid COMPOSE_FILE specified: ${env.COMPOSE_FILE}"
            }
        }
    }
}
